// Auto-generated. Do not edit!

// (in-package scarab_msgs.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;

//-----------------------------------------------------------

class MoveResult {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.final_status = null;
    }
    else {
      if (initObj.hasOwnProperty('final_status')) {
        this.final_status = initObj.final_status
      }
      else {
        this.final_status = 0;
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type MoveResult
    // Serialize message field [final_status]
    bufferOffset = _serializer.uint8(obj.final_status, buffer, bufferOffset);
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type MoveResult
    let len;
    let data = new MoveResult(null);
    // Deserialize message field [final_status]
    data.final_status = _deserializer.uint8(buffer, bufferOffset);
    return data;
  }

  static getMessageSize(object) {
    return 1;
  }

  static datatype() {
    // Returns string type for a message object
    return 'scarab_msgs/MoveResult';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return '79b09be8622e2706904a1b48e955dad6';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    # ====== DO NOT MODIFY! AUTOGENERATED FROM AN ACTION DEFINITION ======
    uint8 FINISHED=0      # Successfully reached goal
    uint8 TIMEOUT=1       # Took too long to reach goal
    uint8 STUCK=2         # Robot has not moved in a while
    uint8 NOTREADY=3      # Don't have all the data we need to move, check back soon!
    uint8 UNREACHABLE=4   # Goal is no longer reachable (e.g., due to map change)
    
    uint8 final_status    # Status of robot when action was ended
    
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new MoveResult(null);
    if (msg.final_status !== undefined) {
      resolved.final_status = msg.final_status;
    }
    else {
      resolved.final_status = 0
    }

    return resolved;
    }
};

// Constants for message
MoveResult.Constants = {
  FINISHED: 0,
  TIMEOUT: 1,
  STUCK: 2,
  NOTREADY: 3,
  UNREACHABLE: 4,
}

module.exports = MoveResult;
